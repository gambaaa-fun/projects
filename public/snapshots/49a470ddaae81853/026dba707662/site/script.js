...
function initGalleries() {
    document.querySelectorAll('.gallery').forEach(gallery => {
        ...
        function showSlide(n) {
            ...
            const thumbs = gallery.closest('.detail-gallery-box')?.querySelectorAll('.thumb-item');
            if (thumbs) {
        ...
        // Bind thumbnail click events to keep state in perfect sync
        const thumbs = gallery.closest('.detail-gallery-box')?.querySelectorAll('.thumb-item');
        if (thumbs) {
...
window.setDetailSlide = function(idx) {
    const gallery = document.querySelector('.product-detail-section .gallery');
    if (!gallery) return;
    const slides = gallery.querySelectorAll('.gallery-slide');
...
// ── Buy panel toggle ───────────────────────────────────────
window.toggleBuyPanel = function(btn) {
    const panel = btn.closest('article').querySelector('.strip-buy-panel');
    const isOpen = panel.classList.toggle('open');
    btn.textContent = isOpen
        ? (LANG === 'en' ? '✕ Close' : '✕ Zavřít')
        : (LANG === 'en' ? 'I want to buy' : 'Chci koupit');
};
